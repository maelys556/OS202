...
"""
chunk_size = height // comm_size
start_index = rank * chunk_size
end_index = start_index + chunk_size if rank < comm_size - 1 else height
"""
...
class Colony:
    ...
    def advance(self, the_maze, pos_food, pos_nest, pheromones, food_counter=0):
        ...
        if loaded_ants.shape[0] > 0:
            # Les fourmis chargées reviennent à la maison
    ...
    def display(self, screen, sprites, historic):
        [screen.blit(sprites[self.directions[i]], (8*historic[i, 1], 8*historic[i, 0])) for i in range(self.directions.shape[0])]
...
if __name__ == "__main__":
    ...
    nb_ants_local = nb_ants//(comm_size-1)
    if rank ==0:
        ants = Colony(nb_ants_local*(comm_size-1), pos_nest, max_life)
    else :
        ants = Colony(nb_ants_local, pos_nest, max_life)
    ...
    while True:
        ...
        if rank==0:
            ...
            datas = [MPI.pickle.loads(comm.recv(source=i))  for i in range(1, comm_size)]
            


            # GESTION DES FOURMIS
            historic_list = [data[1] for data in datas]
            historic = np.vstack(historic_list)

            # GESTION DES PHEROMONES
            all_tab = [data[2] for data in datas]
            
            pherom.pheromon = np.maximum.reduce(all_tab)
            for i in range(1, comm_size):
                comm.send(pherom.pheromon, dest=i)



            # GESTION DU FOOD COUNTER
            food_counter = sum(data[0] for data in datas)
            ...
            ants.display(screen, sprites_ants, historic)
            pg.display.update()
            ...
            av_fps = (counter*av_fps + (end-deb))/(counter + 1)
            counter += 1
            print(f"FPS moyen : {1./(av_fps):6.2f}, nourriture : {food_counter:7d}", end='\r')
            deb = time.time()
        ...
        if rank>=1:
            ...
            data = MPI.pickle.dumps([food_counter,  ants.historic_path[range(0, ants.seeds.shape[0]), ants.age[:], :], pherom.pheromon])
            comm.send(data, dest=0)

            pherom.pheromon = comm.recv(source=0)

    print( "fps moyen : ",1/av_fps)
